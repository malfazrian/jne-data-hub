...
def add_status_pod_2(df, debug: bool = False):
    ...
    # -----------------------------
    # Final Translation to STATUS_POD_2
    # -----------------------------
    def translate(row):
        try:
            if row["Cek_OP_7"] == "Auto Closed Breach (D25,D26)":
                return "Claim Breach (Over SLA)"
            elif row["Cek_OP_7"] == "Auto Closed Destination (CL2,CL4)":
                return "Claim Breach (Over SLA)"
            elif row["Cek_OP_7"] == "Claim Missing":
                return "Claim Missing"
            elif row["Cek_OP_7"] == "Claim Damage":
                return "Claim Damage"
            elif row["Cek_OP_7"] in ["Open POD", "Problem Missroute", "Un Runsheet"]:
                return "On Process"
            elif row["Cek_OP_7"] == "Auto Closed Origin (CL1)":
                return "Un Receiving"
            elif row["Cek_OP_7"] == "RU Shipper/Origin":  # Explicit mapping
                return "RU Origin"
            else:
                return row["Cek_OP_7"]
        except Exception as e:
            print(f"Error in translate for row: {e}")
            return "Error"

    try:
        df.loc[:, "STATUS_POD_2"] = df.apply(translate, axis=1)
        if debug: print("✅ STATUS_POD_2 final dibuat")
        if debug: print(f"STATUS_POD_2 unique values: {df['STATUS_POD_2'].unique().tolist()}")
    except Exception as e:
        raise ValueError(f"Error in translate to STATUS_POD_2: {e}")
    ...
    drop_cols = [
        ...
        if (
            col.startswith(("Status", "CEK_D24_D25", "CGK_", "Status_", "Cek_OP", "3LC_", 
                            "HVOSamaDest", "X_Ray_Cek1", "CHECK_NO_FW",
                            "Status_FW_Bantu", "NEW_STATUS", "DB1_Check", "Check_RT"))
            or col in DROP_EXACT_COLS
        )
    ]
    drop_cols = [c for c in drop_cols if c != "STATUS_POD_2"]
    ...
    def translate(value):
        try:
            if value == "Auto Closed Breach (D25,D26)":
                ...
                return "Un Receiving"
            elif value == "RU Shipper/Origin":  # Explicit mapping
                return "RU Origin"
            else:
                return value
        except Exception as e:
